...
import { Box, Link as MuLink, MenuItem, Tooltip, Typography } from '@mui/material';
      ...
      render: (row: TableConfigData) => (
        <>
          <Box className={Styles.configNameColumn}>
            <MuLink underline="none" component={Link} to={`/config/${row.configName}/${row.version}`}>
    ...
    },
    {
      ...
      render: (row: TableConfigData) => (
        <Box className={Styles.configSchemaColumn}>
          <MuLink underline="none" component={Link} to={`/schema/view?schemaId=${row.schemaId}`}>
